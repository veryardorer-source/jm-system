...
- **RLS 계층형 적용(2026-07-02 최신)**: 초기엔 전체 테이블 RLS ON + `authenticated` 전체허용(`auth_all`)이었으나, 이후 테이블별로 세분화. 데이터 클라이언트는 `createBrowserClient`(쿠키 세션)라 로그인 사용자로 요청되고, RLS가 역할/참여자 기준으로 강제한다.
  ...
  - **partner**: **공개 지정된 현장만**(project_access, 현장 등록 시 선택 + 현장 상세 🔒 외부공개에서 초대/내보내기) 보기 전용 — 미지정 현장은 목록·직접 URL 모두 차단. 채팅/금전/서류 insert·update 불가, 화면도 숨김. 수금(payments)은 admin 전용
...
- **새 DB 세팅 시 SQL 실행 순서** (Supabase SQL Editor):
  ...
  - project_files 정책은 rls_money와 project_access가 **동일 내용**(partner=공개 현장만)이라 어느 쪽을 나중에 실행해도 결과가 같음.
...
- 카테고리: 공사전사진 / 시공사진 / 마감사진 / 도면 / 3D / **견적서 / 계약서** / 미팅내용 / 고객요청 / 구매링크 / 기타
- **견적 관련 범위(중요)**: 본체는 **견적서/계약서 "파일 첨부·보관"까지만**(도면·3D처럼 업로드/보기/공유). 견적서 **작성·공종/단가 입력·자동계산·단가표·엑셀 생성**은 본체에 넣지 않음 → **별도 프로젝트(`estimate-program` 브랜치)**. `/estimates` 라우트·`price_book`·`estimate*.ts`·`exceljs`는 본체 금지.
